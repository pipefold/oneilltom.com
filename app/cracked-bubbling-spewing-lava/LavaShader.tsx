...
export function LavaEffect() {
  ...
  const {
    ...
    displacementScale: { value: 0.4, min: 0, max: 1, step: 0.01 },
  ...
  const vertexShader = `
    ...
    // Eruption function
    float eruption(vec2 pos, float time) {
      ...
      for (int i = 0; i < 3; i++) {
        ...
        // Distance to eruption center
        float dist = distance(pos, eruptionCenter);
    ...
    void main() {
      ...
      float baseNoise = fbm(vec3(uv * uNoiseScale, uTime * uWaveSpeed)) * 0.5;
      
      // Bubble noise (higher frequency, more pronounced)
      float bubbleNoise = snoise(vec3(
        uv.x * uBubbleFrequency,
        uv.y * uBubbleFrequency,
        uTime * uWaveSpeed
      )) * uBubbleHeight;
      
      // Add occasional eruptions
      float eruptionEffect = eruption(uv, uTime) * 2.0;
