...
// Circle packing shader component
export function CirclePackingShader() {
  ...
  const vertexShader = `
    ...
    void main() {
      ...
      float noise = snoise(vUv * 3.0 + uTime * 0.1) * 0.5 + 0.5;
  ...
  const fragmentShader = `
    ...
    void main() {
      ...
      vec2 worleyResult = worley(vUv, uCellDensity);
  ...
  const controls = useControls("Circle Packing", {
    cellDensity: { value: 5, min: 1, max: 20, step: 0.1 },
    circleSize: { value: 0.35, min: 0.1, max: 0.5, step: 0.01 },
    animationSpeed: { value: 0.5, min: 0, max: 2, step: 0.01 },
    displacementStrength: { value: 0.1, min: 0, max: 0.5, step: 0.01 },
    baseColor: "#2a6c8f",
    accentColor: "#f7b538",
  });
